...
def train_graph(data, reverse_dictionary, batch_size, embedding_size, num_sampled, num_skips,
  ...
  with graph.as_default():
    ...
    with tf.compat.v1.name_scope('inputs'):
      train_inputs = tf.keras.Input(batch_input_shape= (batch_size,), dtype=tf.dtypes.int32)
      train_labels = tf.keras.Input(batch_input_shape=(batch_size,1), dtype=tf.dtypes.int32)
    ...
    # Compute the average tf.nn.sampled_softmax_loss for the batch.
    ...
    # embeddings.
    norm = tf.math.sqrt(tf.reduce_sum(tf.math.square(embeddings), 1, keepdims=True))
  ...
  num_steps = 25001 # increase this above 10k
